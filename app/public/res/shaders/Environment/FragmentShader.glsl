...
{
    float weight = max(dot(normal, normalize(uDirectional.direction)), 0.0);
...
vec3 Spot(in vec3 normal)
{
    ...
    for (int i = 0; i < MAX_LIGHTS; ++i)
    {
        if (i < uSpotCount)
        {
            ...
            float angle_weight = pow(dot(normalize(spot.direction), direction), 2.0);
...
vec3 Light()
{
	...
	{
		vec3 vertexNormal = normalize(vNormalVector);
        ...
        return ambient * uMaterial.ambient.rgb;
}
...
vec4 Colour()
{
	...
	if (uMaterial.useTexture)
		colour *= textureCube(uSampler, -reflect(vVertexUV, vVertexNormal));
    else
	   colour *= vVertexColour;
...
void main(void)
{
	...
	gl_FragColor = vec4(Effect(colour.rgb, light), uMaterial.alpha * colour.a);
}