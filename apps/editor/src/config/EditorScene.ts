import { AreaLight, BasicLitMaterial, DirectionalLight, Entity, Game, Mesh, MeshRenderer, PointLight, RenderType, Scene, Script, ScriptSystem, Shader, Transform } from "@fwge/core";
import { InputSystem } from "@fwge/input";
import { EditorRenderSystem } from "./EditorRenderSystem";
import { EditorWindow } from "./EditorWindow";
import { FPSController } from "./entities/FPSController";
import { FullScreen } from "./entities/FullScreen";
import { Platform } from "./entities/Platform";
import type { Vector3Array } from "@fwge/common";
    ...
    override Init(): void
    {
        ...
        const sphereRotator = new Script(
            {
                update()
                {
                    ...
                    const x = scene.x * ((transform.Id % 7) + 1) / 5;
        ...
        this.CreateEntity()
            .AddComponent(new AreaLight(
                {
                    skyBox: { source: '$lib/res/img/clouds.jpg' },
                    colour: [1, 1, 1],
                    intensity: 0.15
                }));
