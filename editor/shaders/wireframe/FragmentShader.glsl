...
vec3 Point(in vec3 normal, in float shininess)
{
    ...
    for (int i = 0; i < MAX_POINT; ++i)
    {
        if (i < U_Point_Count)
        {
            ...
            if (distance <= point.Radius)
            {
                ...
                float specular_weight = pow(max(dot(reflection, eyeVector), 0.0), shininess);
...
vec3 Light()
{
    ...
    if (U_Material.HasSpecular)
        shininess *= texture2D(U_Sampler.Specular, vec2(V_UV.s, V_UV.t)).r;

    Normal = normalize(Normal);
    
    vec3 ambient = (U_Ambient.Colour.rgb * U_Ambient.Intensity);
    
    return ambient + Directional(Normal) + Point(Normal, shininess);
}

vec4 Colour()
{
    return vec4(U_Material.Ambient.rgb, 1.0);
}
